...
# 搭建神经网络
class for6modes_net(nn.Module):
    # 初始化方法
    def __init__(self):
        # 继承nn.Module的初始化方法
        super(for6modes_net, self).__init__()
        ...
        self.linear = Sequential(
            nn.Flatten(),    # 展平层
            nn.Linear(in_features=256*2*2, out_features=2048),    # 全连接层 FC2048（2048个神经元，输出特征数为 2048）
            nn.ReLU(),
            nn.Linear(in_features=2048, out_features=512),    # 全连接层 FC512（512个神经元，输出特征数为 512）
            nn.ReLU(),
            nn.Linear(in_features=512, out_features=11)    # 全连接层 FC11（12个神经元，输出特征数为 11 = 6*振幅 + 5*相位）
        )
...
if __name__ == '__main__':
    ...
    print(f"最终输出形状: {output.shape}") # 应该是 [1, 11]