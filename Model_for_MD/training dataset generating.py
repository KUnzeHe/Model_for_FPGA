...
if __name__ == "__main__":
    ...
    # 生成3modes的训练集和测试集：信号，训练集——40000，测试集——1000
    num_modes_3 = 3  # 注意：在更改种类数量的时候一定要同时改掉model以及quant_basic里的linear2层的输出通道数（out_features）！
    num_samples_3_train = 40000  # as in paper's training case
    num_samples_3_test = 1000  # as in paper's test case
    resolution_3 = 16

    # 生成训练集
    X_3_train, y_amp_3_train, y_phase_3_train = generate_training_data_for_regression(num_modes_3, num_samples_3_train, resolution_3)
    np.savez('Model_for_MD/datasets/3modes_mode_decomposition_dataset_train_regression.npz', X = X_3_train, 
             y_amp = y_amp_3_train, y_phase = y_phase_3_train)  
    # 生成测试集
    X_3_test, y_amp_3_test, y_phase_3_test = generate_training_data_for_regression(num_modes_3, num_samples_3_test, resolution_3)
    np.savez('Model_for_MD/datasets/3modes_mode_decomposition_dataset_test_regression.npz', X = X_3_test, 
             y_amp = y_amp_3_test, y_phase = y_phase_3_test)    
    
    # 生成5modes的训练集和测试集：信号，训练集——100000，测试集——1000
    num_modes_5 = 5  # 注意：在更改种类数量的时候一定要同时改掉model以及quant_basic里的linear2层的输出通道数（out_features）！
    num_samples_5_train = 100000  # as in paper's training case
    num_samples_5_test = 1000  # as in paper's test case
    resolution_5 = 32

    # 生成训练集
    X_5_train, y_amp_5_train, y_phase_5_train = generate_training_data_for_regression(num_modes_5, num_samples_5_train, resolution_5)
    np.savez('Model_for_MD/datasets/5modes_mode_decomposition_dataset_train_regression.npz', X = X_5_train, 
             y_amp = y_amp_5_train, y_phase = y_phase_5_train)      
    # 生成测试集
    X_5_test, y_amp_5_test, y_phase_5_test = generate_training_data_for_regression(num_modes_5, num_samples_5_test, resolution_5)
    np.savez('Model_for_MD/datasets/5modes_mode_decomposition_dataset_test_regression.npz', X = X_5_test, 
             y_amp = y_amp_5_test, y_phase = y_phase_5_test)
    

    # 生成6modes的训练集和测试集：信号，训练集——100000，测试集——1000
    num_modes_6 = 6  # 注意：在更改种类数量的时候一定要同时改掉model以及quant_basic里的linear2层的输出通道数（out_features）！
    num_samples_6_train = 100000  # as in paper's training case 
    num_samples_6_test = 1000  # as in paper's test case
    resolution_6 = 32

    # 生成训练集
    X_6_train, y_amp_6_train, y_phase_6_train = generate_training_data_for_regression(num_modes_6, num_samples_6_train, resolution_6)
    np.savez('Model_for_MD/datasets/6modes_mode_decomposition_dataset_train_regression.npz', X = X_6_train, 
             y_amp = y_amp_6_train, y_phase = y_phase_6_train)  
    # 生成测试集
    X_6_test, y_amp_6_test, y_phase_6_test = generate_training_data_for_regression(num_modes_6, num_samples_6_test, resolution_6)
    np.savez('Model_for_MD/datasets/6modes_mode_decomposition_dataset_test_regression.npz', X = X_6_test, 
             y_amp = y_amp_6_test, y_phase = y_phase_6_test)    
